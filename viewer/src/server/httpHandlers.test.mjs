...
test("CAD Viewer API middleware handles aircraft pipeline generation and status requests", async () => {
  const middleware = createCadViewerApiMiddleware({
    backend: {
      readCatalog: async () => ({ schemaVersion: 4, entries: [] }),
      resolveRoot: (dir) => ({ dir: "outputs", rootPath: "C:/fake/outputs", rootName: "outputs" }),
    },
  });

  // 1. Rejects GET on generate
  const getReq = { method: "GET", url: "/__cad/pipeline/aircraft/generate" };
  const getRes = createResponse();
  await middleware(getReq, getRes, () => {});
  assert.equal(getRes.statusCode, 405);

  // 2. Accepts POST on generate
  const postReq = createJsonRequest({
    method: "POST",
    url: "/__cad/pipeline/aircraft/generate",
    body: {
      aircraft_id: "FW-007",
      project_id: "PROJ-1",
      version: "v1",
  ...
  assert.equal(data.aircraft_id, "FW-007");
  assert.equal(data.version, "v1");
  assert.equal(data.prototype_readiness, "READY");

  // 3. Status endpoint
  const statReq = { method: "GET", url: "/__cad/pipeline/aircraft/status?aircraft=FW-007&version=v1" };
  const statRes = createResponse();
  await middleware(statReq, statRes, () => {});
  assert.equal(statRes.statusCode, 200);
  const statData = JSON.parse(statRes.body);
  assert.equal(statData.ok, true);
  assert.equal(statData.aircraft_id, "FW-007");
});
